export default {
  ...
  run: async (client, m, args) => {
    ...
    for (const jid of Object.keys(chat.users)) {
      ...
      chat.characters =
        typeof chat.characters === "object" ? chat.characters : {};
      ...
      userList.push(
        `@${jid.split("@")[0]} *[${u.characters.length}]* - ${tiempo}`,
      );
      mentions.push(jid);
    }
    if (!eliminados)
      return m.reply(
        "ꕥ No se encontraron usuarios inactivos.\n> ⴵ Tiempo limite: 40 dias",
      );
    const report = [
      "❀ Se han eliminado los datos de los usuarios inactivos.",
      `> ♡ Claims eliminados: ${waifus}`,
      `> ⛁ Coins eliminados: ${dinero.toLocaleString()}`,
      `> ❖ Usuarios inactivos: ${eliminados}`,
      `> ⴵ Tiempo límite: 40 días`,
      `> ❏ El comando se ejecutó en ${getDuration(start)}ms`,
      "",
      ...userList,
    ].join("\n");
