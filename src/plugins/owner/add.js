...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    try {
      ...
      const who2 =
        mentioned.length > 0 ? mentioned[0] : m.quoted ? m.quoted.sender : null;
      ...
      if (command === "addcoin") {
        ...
        if (!coinTxt)
          return client.reply(
            m.chat,
            "ꕥ Por favor, ingresa la cantidad que deseas añadir.\nEjemplo: !addcoin @usuario 100",
            m,
          );
        if (isNaN(coinTxt))
          return client.reply(m.chat, "ꕥ Solo se permiten números.", m);
        await m.react("🕒");
        ...
        return client.reply(
          m.chat,
          `❀ *Añadido:*\n» ${dmt} ${currency}\n@${who.split("@")[0]}, recibiste ${dmt} ${currency}`,
          m,
          { mentions: [who] },
        );
      }
      if (command === "addxp") {
        ...
        if (!xpTxt)
          return client.reply(
            m.chat,
            "ꕥ Ingresa la cantidad de experiencia (XP) que deseas añadir.\nEjemplo: !addxp @usuario 50",
            m,
          );
        if (isNaN(xpTxt))
          return client.reply(m.chat, "ꕥ Solo números son permitidos.", m);
        await m.react("🕒");
        ...
        if (xp < 1) {
          ...
          return client.reply(
            m.chat,
            "ꕥ El mínimo de experiencia (XP) es *1*",
            m,
          );
        }
