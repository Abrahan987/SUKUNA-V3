    ...
    try {
      ...
      for (const p of anu.participants) {
        ...
        const pp = await client
          ...
          .catch((_) => "https://cdn.yuki-wabot.my.id/files/nufq.jpeg");
        const mensajes = {
          add: chat.sWelcome
            ? `\n┊➤ ${chat.sWelcome
  ...
  client.ev.on("messages.upsert", async ({ messages }) => {
    ...
    if (m.messageStubType == 21) {
      await client.sendMessage(id, {
        text: `「✎」 @${phone} cambió el nombre del grupo a *${m.messageStubParameters[0]}*`,
    ...
    if (m.messageStubType == 25) {
      await client.sendMessage(id, {
        text: `「✎」 @${phone} cambió los ajustes del grupo para permitir que ${m.messageStubParameters[0] == "on" ? "solo admins" : "todos"} puedan configurar el grupo.`,
        mentions: [actor, ...groupAdmins.map((v) => v.id)],
      });
    }
    if (m.messageStubType == 26) {
      await client.sendMessage(id, {
        text: `「✎」 @${phone} cambió los ajustes del grupo para permitir que ${m.messageStubParameters[0] === "on" ? "solo los administradores puedan enviar mensajes al grupo." : "todos los miembros puedan enviar mensajes al grupo."}`,
        mentions: [actor, ...groupAdmins.map((v) => v.id)],
      });
    }
  });
