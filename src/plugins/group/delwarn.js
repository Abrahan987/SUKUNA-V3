...
export default {
  ...
  run: async (client, m, args) => {
    ...
    if (!who2)
      return m.reply(
        "《✧》 Debes mencionar o responder al usuario cuya advertencia deseas eliminar.",
      );
    const targetId = await resolveLidToRealJid(who2, client, m.chat);
    const user = chat.users[targetId];
    if (!user)
      return m.reply("《✧》 No se encontró al usuario en la base de datos.");
    ...
    if (isNaN(index)) {
      return m.reply(
        "《✧》 Debes especificar el índice de la advertencia que deseas eliminar o usar all para borrar todas.",
      );
    }
