export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    if (!m.mentionedJid[0] && !m.quoted) {
      return m.reply('☢︎︎ Etiqueta o responde al *mensaje* de la persona que deseas eliminar del grupo.', m)
    }
    ...
    if (!participant) {
      return client.reply(m.chat, `☼︎ @${user.split('@')[0]} ya no forma parte del grupo.`, m, { mentions: [user] })
    }
    ...
    try {
      ...
      client.reply(m.chat, `☹︎ @${user.split('@')[0]} ha sido *eliminado* exitosamente.`, m, { mentions: [user] })
    } catch (e) {
