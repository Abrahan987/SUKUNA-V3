export default {
  ...
  run: async (client, m, args) => {
    ...
    if (!value) {
      return m.reply(
        `❀ Ingresa el enlace de un canal de WhatsApp.\n\nEjemplo:\n*${m.usedPrefix}setchannel* https://whatsapp.com/channel/XXXXXXXXXXXXXX`,
      );
    }
    const channelUrl = value.match(
      /(?:https:\/\/)?(?:www\.)?(?:chat\.|wa\.)?whatsapp\.com\/channel\/([0-9A-Za-z]{22,24})/i,
    )?.[1];
    ...
    return m.reply(
      `❀ Se cambió el canal del Socket a *"${config.nameid}"* correctamente.`,
    );
  },
};
