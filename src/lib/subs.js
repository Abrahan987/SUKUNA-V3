...
export async function startSubBot(
  ...
  sock.ev.on(
    "connection.update",
    async ({ connection, lastDisconnect, isNewLogin, qr }) => {
      ...
      if (connection === "close") {
        ...
        if ([401, 403].includes(reason)) {
          if (intentos < 5) {
            ...
            console.log(
              chalk.gray(
                `[ ✿  ]  SUB-BOT ${botId} Falló tras 5 intentos. Eliminando sesión.`,
              ),
            );
            try {
              ...
              console.error(
                `[ ✿  ] No se pudo eliminar la carpeta ${sessionFolder}:`,
                e,
              );
            }
