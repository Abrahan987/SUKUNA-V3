...
function initDB(m, client) {
  ...
  settings.self ??= false;
  settings.prefix ??= ["/", "!", ".", "#"];
  settings.commandsejecut ??= isNumber(settings.commandsejecut)
  ...
  chat.users[m.sender].lastCmd = isNumber(chat.users[m.sender].lastCmd)
    ? chat.users[m.sender].lastCmd
    : 0;
  chat.users[m.sender].coins = isNumber(chat.users[m.sender].coins)
    ? chat.users[m.sender].coins
    : 0;
  chat.users[m.sender].bank = isNumber(chat.users[m.sender].bank)
    ? chat.users[m.sender].bank
    : 0;
  chat.users[m.sender].characters = Array.isArray(
    chat.users[m.sender].characters,
  )
    ? chat.users[m.sender].characters
    : [];
}
