...
export async function smsg(client, m, store) {
  ...
  if (m.message) {
    ...
    m.command =
      m.body && m.body.replace(m.usedPrefix, "").trim().split(/ +/).shift();
    m.args =
      m.body
        ?.trim()
        .replace(
  ...
  client.sendAlbumMessage = async (jid, medias, options = {}) => {
    ...
    for (let i = 0; i < medias.length; i++) {
      ...
      await client.relayMessage(mediaMsg.key.remoteJid, mediaMsg.message, {
