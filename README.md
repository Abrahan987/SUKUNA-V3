...
```
```bash
git clone https://github.com/Abrahan987/SukunaV3 && cd SukunaV3
```
```bash
yarn install
```
...
> *Ejecutar estos comandos dentro de la carpeta SukunaV3*
```bash
...
```bash
cd && cd SukunaV3 && rm -rf sessions/Principal && npm run qr
```
----
### Obtener nuevo código de teléfono
```bash
cd && cd SukunaV3 && rm -rf sessions/Principal && npm run code
```
</details>

<details>
<summary><b>❀ Actualizar SukunaV3</b></summary>
...
**Comandos para actualizar SukunaV3 de forma automática**

```bash
grep -q 'bash\|wget' <(dpkg -l) || apt install -y bash wget && wget -O - https://raw.githubusercontent.com/Abrahan987/SukunaV3/master/termux.sh | bash
```
...
```bash
cd && cd SukunaV3 && nano settings.js
```
...
### **`❀ Colaboradores`**
<a href="https://github.com/Abrahan987/SukunaV3/graphs/contributors">
<img src="https://contrib.rocks/image?repo=Abrahan987/SukunaV3" />
</a>
